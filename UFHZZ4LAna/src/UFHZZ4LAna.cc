...
private:
    ...
    vector<float>  jet_DeepJet_b, jet_DeepJet_bb, jet_DeepJet_lepb, jet_DeepJet_c, jet_DeepJet_uds, jet_DeepJet_g, jet_DeepJet_CvsL, jet_DeepJet_CvsB;
    vector<float>  jet_ParticleNet_b, jet_ParticleNet_bb, jet_ParticleNet_c, jet_ParticleNet_cc, jet_ParticleNet_uds, jet_ParticleNet_g, jet_ParticleNet_undef, jet_ParticleNet_pu, jet_ParticleNet_CvsL, jet_ParticleNet_CvsB;
    ...
    // SV
    vector<float>  sv_pt, sv_eta, sv_phi, sv_mass, sv_energy, sv_ntracks;
...
//
    ...
    */
    ...
    // GEN particles
    vector<double> GENPart_pt; vector<double> GENPart_eta; vector<double> GENPart_phi; vector<double> GENPart_mass;
    vector<int> GENPart_id; vector<int> GENPart_status;
...
//    Mela* GENmela;// FIXME
    ...
    vector<float> GENPart_pt_float, GENPart_eta_float;
    ...
    edm::EDGetTokenT<std::vector<reco::VertexCompositePtrCandidate> > svSrc_;
    edm::EDGetTokenT<edm::View<pat::Jet> > svPhantomJetSrc_;
...
UFHZZ4LAna::UFHZZ4LAna(const edm::ParameterSet& iConfig) :
    ...
    svSrc_(consumes<std::vector<reco::VertexCompositePtrCandidate> >(iConfig.getUntrackedParameter<edm::InputTag>("svSrc"))),
    svPhantomJetSrc_(consumes<edm::View<pat::Jet> >(edm::InputTag("pfParticleNetSVFlavourTagsPhantomJets"))),
    ...
    storePFCands(iConfig.getUntrackedParameter<bool>("storePFCands",false))
{
    ...
    if (year_ == -2016)
        elec_scalefac_Cracks_name_161718[0]="ElectronSF_UL2016preVFP_gap.root";
...
UFHZZ4LAna::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    // SV and its phantom jet collection
    edm::Handle<std::vector<reco::VertexCompositePtrCandidate> > svs;
    iEvent.getByToken(svSrc_, svs);
    edm::Handle<edm::View<pat::Jet> > sv_phantom_jets;
    iEvent.getByToken(svPhantomJetSrc_, sv_phantom_jets);
    ...
    cosTheta1=9999.0; cosTheta2=9999.0; cosThetaStar=9999.0; Phi=9999.0; Phi1=9999.0;pT4lj=-1.0; pT4ljj=-1.0; pT4lj_2p5=-1.0; pT4ljj_2p5=-1.0;
    mass3l=-1.0;
    massH_vtx_chi2=999;
    ...
    jet_DeepJet_b.clear(); jet_DeepJet_bb.clear(); jet_DeepJet_lepb.clear(); jet_DeepJet_c.clear();
    jet_DeepJet_uds.clear(); jet_DeepJet_g.clear(); jet_DeepJet_CvsL.clear(); jet_DeepJet_CvsB.clear();
    jet_ParticleNet_b.clear(); jet_ParticleNet_bb.clear(); jet_ParticleNet_c.clear(); jet_ParticleNet_cc.clear(); jet_ParticleNet_uds.clear();
    jet_ParticleNet_g.clear(); jet_ParticleNet_undef.clear(); jet_ParticleNet_pu.clear(); jet_ParticleNet_CvsL.clear(); jet_ParticleNet_CvsB.clear();
...
//    pTj1=-1; 
    ...
    // SV
    sv_pt.clear(); sv_eta.clear(); sv_phi.clear(); sv_mass.clear();
    sv_energy.clear(); sv_ntracks.clear(); sv_normchi2.clear(); sv_dxy.clear();
 ...
 */
    ...
    */
    ...
    GENcosTheta1=9999.0; GENcosTheta2=9999.0; GENcosThetaStar=9999.0; GENPhi=9999.0; GENPhi1=9999.0; GENpT4lj=-1.0; GENpT4ljj=-1.0;
    ...
    // GEN particles
    GENPart_id.clear(); GENPart_pt.clear(); GENPart_eta.clear(); GENPart_phi.clear(); GENPart_mass.clear();
    ...
    if(theVertex >= 0 && (isMC || (!isMC && trigConditionData)) )  {
        ...
        if( (recoMuons.size() + recoElectrons.size()) >= (uint)skimLooseLeptons ) {
            ...
            if ( ntight >= (uint)skimTightLeptons ) {
                ...
                // custom for Higgs+charm: SV variables
                const reco::Vertex & pv = *PV;
                for(unsigned int i = 0; i < svs->size(); ++i) {

                    const reco::VertexCompositePtrCandidate & sv = svs->at(i);
                    const pat::Jet & spjet = sv_phantom_jets->at(i);
                    sv_pt.push_back(sv.pt());
                    sv_eta.push_back(sv.eta());
                    sv_phi.push_back(sv.phi());
                    sv_mass.push_back(sv.mass());
                    sv_energy.push_back(sv.energy());

                    // sv properties
                    sv_ntracks.push_back(sv.numberOfDaughters());
                    sv_normchi2.push_back(sv.vertexNormalizedChi2());
                    ...
                    sv_costhetasvpv.push_back(p.Unit().Dot(d.Unit()));

                    // SV tagging scores and truth flavour labels
                    sv_genflavour.push_back(spjet.hasUserFloat("gen_flavour") ? spjet.userFloat("gen_flavour") : -1);
                    sv_nbhadrons.push_back(spjet.hasUserFloat("n_bhadrons") ? spjet.userFloat("n_bhadrons"): -1);
                    sv_nchadrons.push_back(spjet.hasUserFloat("n_chadrons") ? spjet.userFloat("n_chadrons"): -1);
                    sv_ParticleNet_b.push_back(spjet.userFloat("probb"));
                    sv_ParticleNet_bb.push_back(spjet.userFloat("probbb"));
                    sv_ParticleNet_c.push_back(spjet.userFloat("probc"));
                    sv_ParticleNet_cc.push_back(spjet.userFloat("probcc"));
...
//next
    ...
    GENPart_pt_float.clear(); GENPart_pt_float.assign(GENPart_pt.begin(),GENPart_pt.end());
    GENPart_eta_float.clear(); GENPart_eta_float.assign(GENPart_eta.begin(),GENPart_eta.end());
    GENPart_phi_float.clear(); GENPart_phi_float.assign(GENPart_phi.begin(),GENPart_phi.end());
    GENPart_mass_float.clear(); GENPart_mass_float.assign(GENPart_mass.begin(),GENPart_mass.end());
...
{     
    ...
    tree->Branch("jet_DeepJet_b",&jet_DeepJet_b);
    tree->Branch("jet_DeepJet_bb",&jet_DeepJet_bb);
    tree->Branch("jet_DeepJet_lepb",&jet_DeepJet_lepb);
    tree->Branch("jet_DeepJet_c",&jet_DeepJet_c);
    tree->Branch("jet_DeepJet_uds",&jet_DeepJet_uds);
    tree->Branch("jet_DeepJet_g",&jet_DeepJet_g);
    tree->Branch("jet_DeepJet_CvsL",&jet_DeepJet_CvsL);
    tree->Branch("jet_DeepJet_CvsB",&jet_DeepJet_CvsB);
    tree->Branch("jet_ParticleNet_b",&jet_ParticleNet_b);
    tree->Branch("jet_ParticleNet_bb",&jet_ParticleNet_bb);
    tree->Branch("jet_ParticleNet_c",&jet_ParticleNet_c);
    tree->Branch("jet_ParticleNet_cc",&jet_ParticleNet_cc);
    tree->Branch("jet_ParticleNet_uds",&jet_ParticleNet_uds);
    tree->Branch("jet_ParticleNet_g",&jet_ParticleNet_g);
    tree->Branch("jet_ParticleNet_undef",&jet_ParticleNet_undef);
    tree->Branch("jet_ParticleNet_pu",&jet_ParticleNet_pu);
    tree->Branch("jet_ParticleNet_CvsL",&jet_ParticleNet_CvsL);
    tree->Branch("jet_ParticleNet_CvsB",&jet_ParticleNet_CvsB);
    ...
    tree->Branch("sv_eta",&sv_eta);
    tree->Branch("sv_phi",&sv_phi);
    tree->Branch("sv_mass",&sv_mass);
    tree->Branch("sv_energy",&sv_energy);
    tree->Branch("sv_ntracks",&sv_ntracks);
    tree->Branch("sv_normchi2",&sv_normchi2);
    tree->Branch("sv_dxy",&sv_dxy);
    tree->Branch("sv_dxysig",&sv_dxysig);
    tree->Branch("sv_d3d",&sv_d3d);
    tree->Branch("sv_d3dsig",&sv_d3dsig);
    tree->Branch("sv_costhetasvpv",&sv_costhetasvpv);
    tree->Branch("sv_genflavour",&sv_genflavour);
    tree->Branch("sv_nbhadrons",&sv_nbhadrons);
    tree->Branch("sv_nchadrons",&sv_nchadrons);
    tree->Branch("sv_ParticleNet_b",&sv_ParticleNet_b);
    tree->Branch("sv_ParticleNet_bb",&sv_ParticleNet_bb);
...
//
    ...
    */
    ...
    // GEN particles
    tree->Branch("GENPart_pt",&GENPart_pt_float);
    tree->Branch("GENPart_eta",&GENPart_eta_float);
    tree->Branch("GENPart_phi",&GENPart_phi_float);
    tree->Branch("GENPart_mass",&GENPart_mass_float);
    tree->Branch("GENPart_id",&GENPart_id);
    tree->Branch("GENPart_status",&GENPart_status);
    tree->Branch("GENPart_isExcite",&GENPart_isExcite);
    tree->Branch("GENPart_isCFromBHad",&GENPart_isCFromBHad);
                                  ...
                                  edm::Handle<edm::View<pat::Jet> > &jets)
{
    ...
    for( unsigned int k = 0; k < goodJets.size(); k++) {
        ...
        if (jet_jer->Pt() > jetPtMinCut && fabs(goodJets[k].eta())<4.7) {
            ...
            // for Higgs+charm: more tagging vars
            jet_DeepJet_b.push_back(goodJets[k].bDiscriminator("pfDeepFlavourJetTags:probb"));
            jet_DeepJet_bb.push_back(goodJets[k].bDiscriminator("pfDeepFlavourJetTags:probbb"));
            jet_DeepJet_lepb.push_back(goodJets[k].bDiscriminator("pfDeepFlavourJetTags:problepb"));
            jet_DeepJet_c.push_back(goodJets[k].bDiscriminator("pfDeepFlavourJetTags:probc"));
            jet_DeepJet_uds.push_back(goodJets[k].bDiscriminator("pfDeepFlavourJetTags:probuds"));
            jet_DeepJet_g.push_back(goodJets[k].bDiscriminator("pfDeepFlavourJetTags:probg"));
            jet_DeepJet_CvsL.push_back(jet_DeepJet_c.back()/(jet_DeepJet_c.back()+jet_DeepJet_uds.back()+jet_DeepJet_g.back()+1e-8));
            jet_DeepJet_CvsB.push_back(jet_DeepJet_c.back()/(jet_DeepJet_c.back()+jet_DeepJet_b.back()+jet_DeepJet_bb.back()+jet_DeepJet_lepb.back()+1e-8));
            jet_ParticleNet_b.push_back(goodJets[k].bDiscriminator("pfParticleNetAK4JetTags:probb"));
            jet_ParticleNet_bb.push_back(goodJets[k].bDiscriminator("pfParticleNetAK4JetTags:probbb"));
            jet_ParticleNet_c.push_back(goodJets[k].bDiscriminator("pfParticleNetAK4JetTags:probc"));
            jet_ParticleNet_cc.push_back(goodJets[k].bDiscriminator("pfParticleNetAK4JetTags:probcc"));
            jet_ParticleNet_uds.push_back(goodJets[k].bDiscriminator("pfParticleNetAK4JetTags:probuds"));
            jet_ParticleNet_g.push_back(goodJets[k].bDiscriminator("pfParticleNetAK4JetTags:probg"));
            jet_ParticleNet_undef.push_back(goodJets[k].bDiscriminator("pfParticleNetAK4JetTags:probundef"));
            jet_ParticleNet_pu.push_back(goodJets[k].bDiscriminator("pfParticleNetAK4JetTags:probpu"));
            jet_ParticleNet_CvsL.push_back(goodJets[k].bDiscriminator("pfParticleNetAK4DiscriminatorsJetTags:CvsL"));
            jet_ParticleNet_CvsB.push_back(goodJets[k].bDiscriminator("pfParticleNetAK4DiscriminatorsJetTags:CvsB"));
...
/////// set new obs.
    ...
    if (njets_pt30_eta4p7 > 1) { 
        ...
        phij2=Jet2.Phi();
        mj1j2=(Jet1+Jet2).M();
        mass4ljj=(HVec+Jet1+Jet2).M();
        pT4ljj=(HVec+Jet1+Jet2).Pt();
    ...
    if (njets_pt30_eta4p7_jesup > 1) {
        ...
        mass4ljj_jesup=(HVec+Jet1_jesup+Jet2_jesup).M();
        pT4ljj_jesup=(HVec+Jet1_jesup+Jet2_jesup).Pt();
...
// JES dn
    ...
    if (njets_pt30_eta4p7_jesdn > 1) {
        ...
        mass4ljj_jesdn=(HVec+Jet1_jesdn+Jet2_jesdn).M();
        pT4ljj_jesdn=(HVec+Jet1_jesdn+Jet2_jesdn).Pt();
                                 ...
                                 edm::Handle<reco::JetFlavourInfoMatchingCollection> jetFlavourInfos)
{
    ...
    for(genPart = prunedgenParticles->begin(); genPart != prunedgenParticles->end(); genPart++) {
        ...
        // custom for Higgs+charm analysis
        // bottom/charm flavour hadrons and partons
        bool store_genPart = false;
        if (genPart->isLastCopy()) {
            if ((abs(genPart->pdgId())==5 || CandMCTagUtils::hasBottom(*genPart)) || (abs(genPart->pdgId())==4 || CandMCTagUtils::hasCharm(*genPart))) {
                store_genPart = true;
            }
        }
        if(store_genPart) {
            auto isExcite = [&](const auto p) {
                // only applicable to b/c hadrons
                if (abs(p->pdgId())==4 || abs(p->pdgId())==5) { return false; }
                bool isBFlav = CandMCTagUtils::hasBottom(*p);
                bool isCFlav = CandMCTagUtils::hasCharm(*p);
                for (size_t i=0; i < p->numberOfDaughters(); ++i) {
                    if (isBFlav && CandMCTagUtils::hasBottom( *(p->daughter(i)) ) ) { return true; }
                    if (isCFlav && CandMCTagUtils::hasCharm( *(p->daughter(i)) ) ) { return true; }
                }
                return false;
            };
            auto isCFromBHad = [&](const auto p) {
                // only applicable to b/c hadrons
                if (abs(p->pdgId())==4 || abs(p->pdgId())==5) { return false; }
                if (CandMCTagUtils::hasCharm(*p)) {
                    for (size_t i=0; i < p->numberOfMothers(); ++i) {
                        if (CandMCTagUtils::hasBottom( *(p->mother(i))) ) { return true; }
                    }
                }
                return false;
            };
            GENPart_id.push_back(genPart->pdgId());
            ...
            GENPart_status.push_back(genPart->status());
            GENPart_isExcite.push_back(isExcite(genPart));
            GENPart_isCFromBHad.push_back(isCFromBHad(genPart));
        }
    }
...
//***************************************
        ...
        if (verbose)
            std::cout<<"N_GEN_goodJet: "<<GEN_goodJets.size()<<"; N_GENnjets_pt30_eta4p7: "<<GENnjets_pt30_eta4p7<<"; GEN_TauC_Inc_0j_EnergyWgt: "<<GEN_TauC_Inc_0j_EnergyWgt<<std::endl;
        CalculateNJettinessVar.New_GetRapidityWeightedValues_pTWeighted_UsingEnergy(
             0,  // NJettiness, // this depends on the jettiness that we would like to use
             GEN_goodJets,   // Fixed don't change
             0.0,
