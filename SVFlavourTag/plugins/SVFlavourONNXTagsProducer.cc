...
void SVFlavourONNXTagsProducer::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  ...
  desc.add<edm::InputTag>("src", edm::InputTag("pfParticleNetSVFlavourTagInfos"));
  desc.add<edm::InputTag>("phantom_jets", edm::InputTag("pfParticleNetSVFlavourTagInfos", "svPhantomJets"));
  ...
  desc.add<edm::FileInPath>("model_path",
                            edm::FileInPath("UFHZZAnalysisRun2/SVFlavourTag/data/ParticleNetSV/V02/model.onnx"));
...
void SVFlavourONNXTagsProducer::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  if (debug_) {
    ...
    for (unsigned jet_n = 0; jet_n < tag_infos->size(); ++jet_n) {
      const auto &jet_ref = tag_infos->at(jet_n).jet();
      std::cout << " - Jet #" << jet_n << ", gen_flavour=" << outJets->at(jet_n).userFloat("gen_flavour")
                << ", n_b=" << outJets->at(jet_n).userFloat("n_bhadrons")
                << ", n_c=" << outJets->at(jet_n).userFloat("n_chadrons") << std::endl;
      for (std::size_t flav_n = 0; flav_n < flav_names_.size(); ++flav_n) {
